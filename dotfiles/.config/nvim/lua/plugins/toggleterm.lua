...
local function create_ranger_terminal()
	local ranger_tmpfile = vim.fn.tempname()

	return Terminal:new({
		cmd = 'ranger --choosefiles="' .. ranger_tmpfile .. '" --selectfile="' .. vim.api.nvim_buf_get_name(0) .. '"',
		hidden = true,
...
function _RANGER_TOGGLE()
	local ranger = create_ranger_terminal()
	ranger:toggle()
end

local python = gen_cmd("python")
...
function _G.set_terminal_keymaps()
	...
	vim.keymap.set("t", "<leader>tp", "<cmd>lua _RANGER_TOGGLE()<CR>", opts)
